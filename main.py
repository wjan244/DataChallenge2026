...
if __name__ == "__main__":
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id,run_name=f"{timestamp}_{MODEL_NAME}_domain adaptation"):
        ...
        run_evaluation(timestamp=timestamp,df_val=df_val_samp,method_FT=CONFIG_DOMAINE["method_FT"],prefix="1_domain_adaptation_samp")
        run_evaluation(timestamp=timestamp,df_val=df_val_raw,method_FT=CONFIG_DOMAINE["method_FT"],prefix="1_domain_adaptation_raw")
    ...
    with mlflow.start_run(experiment_id=experiment_id,run_name=f"{timestamp}_{MODEL_NAME}_FT linear probing"):
        ...
        run_evaluation(timestamp=timestamp,df_val=df_val_samp,method_FT=CONFIG_LINEAR_PROBING["method_FT"],prefix="2_linear_probing_samp")
        run_evaluation(timestamp=timestamp,df_val=df_val_raw,method_FT=CONFIG_LINEAR_PROBING["method_FT"],prefix="2_linear_probing_raw")
    ...
    with mlflow.start_run(experiment_id=experiment_id,run_name=f"{timestamp}_{MODEL_NAME}_LORA_FT"):
        ...
        run_evaluation(timestamp=timestamp,df_val=df_val_samp,method_FT=CONFIG_LORA_FT["method_FT"],prefix="3_LoRA_samp")
        run_evaluation(timestamp=timestamp,df_val=df_val_raw,method_FT=CONFIG_LORA_FT["method_FT"],prefix="3_LoRA_raw")
