...
def run_optuna_lp(file_name, timestamp, experiment_id):
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=f"optuna_{timestamp}") as parent:
        ...
        study.optimize(
            lambda trial: objective(trial, cfg, experiment_id),
            n_trials=n_trials,
        ...
        cols = ["lp_loss"] + [c for c in df.columns if c not in ("lp_loss", "best_score")] + ["best_score"]
        df = df[cols].sort_values("best_score")
