...
def setup_probing(model: nn.Module, rank: int = 8, alpha: int = 16, 
                   dropout: float = 0.0, probing_type:str=None,hidden_size:str=None) -> nn.Module:
...
def setup_domain_adaptation(model: nn.Module, rank: int = 8, alpha: int = 16, dropout: float = 0.0, 
                             probing_type:str=None,hidden_size:str=None) -> nn.Module:
...
def setup_lora_finetuning(model: nn.Module, rank: int = 8, alpha: int = 16, dropout: float = 0.0) -> nn.Module:
    # geler/ libérer les poids souhaités
    for name, param in model.named_parameters():
