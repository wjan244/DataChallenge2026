...
def run_adversarial_probing(cfg, timestamp, experiment_id, precedent_run_id=None, precedent_method=None):
    ...
    method_ft                = cfg_method.get("method_FT", "adversarial_probing_training")
    ...
    if cfg_method["run_execution"] == True:
        ...
        with mlflow.start_run(experiment_id=experiment_id, run_name=f"{timestamp}_{cfg_mod}_adversarial_probing_training") as run:
            ...
            # save predictions for the train split using the method tag (e.g. adversarial_probing_training)
            save_split_predictions(timestamp, train_loader, "train", method_ft, cfg_mod, cfg_method.get("method_kwargs"))

        print(f"fin d'entrainement par adversarial_probing_training")
