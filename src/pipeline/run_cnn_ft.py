...
# ---------------------------------------------------------------------------
# Shared helpers (also imported by run_scratch)
# ---------------------------------------------------------------------------
...
def _compile_model(model, cfg_glob):
    if not cfg_glob.get("COMPILE", False):
        return model
    print("Compiling model")
    return torch.compile(model, backend="aot_eager") if DEVICE.type == "mps" else torch.compile(model)


def _build_loss_fn(cfg_method):
...
def _build_loaders(cfg_glob, cfg_method, cfg_mod):
    train_fn = globals()[cfg_method.get("loader_factory", "get_challenge_train_loader")]
    val_fn = globals()[cfg_method.get("val_loader_factory", "get_challenge_val_loader")]
...
# ---------------------------------------------------------------------------
# Core training loop (also imported by run_scratch)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CNN finetuning pipeline (pretrained backbone, progressive unfreezing)
# ---------------------------------------------------------------------------
