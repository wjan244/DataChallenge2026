...
def run_lora(cfg, timestamp, experiment_id, precedent_run_id=None, precedent_method=None):
    ...
    if cfg_method["run_execution"] == True:
        ...
        with mlflow.start_run(experiment_id=experiment_id, run_name=f"{timestamp}_{cfg_mod}_lora_training") as run:
            ...
            # save predictions for the train split using the method tag (e.g. lora_training)
            save_split_predictions(timestamp, train_loader, "train", method_ft, cfg_mod, cfg_method.get("method_kwargs"))

        print(f"fin d'entrainement par lora_training")
