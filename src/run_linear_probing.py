...
def run_linear_probin(cfg,timestamp, experiment_id, precedent_run_id=None, precedent_method=None):
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=f"{timestamp}_{cfg_mod}_linear_probing"):
        ...
        cfg_method_lp.pop("methode_FT", None)
        ...
        run_evaluation(timestamp=timestamp, val_loader=val_loader, method_FT=cfg_method["method_FT"],cfg_glob=cfg_glob,prefix=None)
        return_method = cfg_method["method_FT"]
        test_loader = get_challenge_test_loader(df_test, cfg_glob["BATCH_SIZE"], NUM_WORKERS)
        run_test(timestamp, test_loader, "linear_probing")
