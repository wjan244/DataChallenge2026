...
def run_xgb(cfg):
    ...
    df_val_out = val_ds.meta[["filename", "FaceOcclusion", "gender"]].copy()
    df_val_out["pred"] = preds_val
    df_val_out[["filename", "FaceOcclusion", "pred", "gender"]].to_csv(out_dir / "val.csv", index=False)
    print(f"Val CSV saved → {out_dir}/val.csv  ({len(df_val_out)} rows)")
    ...
    if test_meta_path.exists():
        ...
        print("No test_meta.csv found — skipping test submission.")
